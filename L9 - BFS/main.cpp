...
enum{
    ...
    COMMAND_PATH,
    COMMAND_PERF,
    COMMAND_KNIGHT //added knight command
};
...
void demoKnightMoves()
{
    ...
    char board[8][8];
    memset(board, '.', sizeof(board)); // '.' for unvisited
    board[0][0] = 'K';                 // 'K' for knight's starting position
    ...
    while (first < last) 
    {
        ...
        printf("\n");
        sleep(1); // Slow-motion effect for visualization

        int currR = q[first][0], currC = q[first][1];
        first++;
        board[currR][currC] = '*'; // Mark the current position as visited

        // Process all possible knight moves
        for (int i = 0; i < 8; i++) 
        {
            int newR = currR + knightMove[i][0];
            int newC = currC + knightMove[i][1];

            if (validPoz(newR, newC) && visited[newR][newC] == 0) 
            {
                visited[newR][newC] = 1; // Mark as visited
                q[last][0] = newR;
                q[last][1] = newC;
                last++;
                board[newR][newC] = 'K'; // Move the knight
            }
        }
    }
    ...
    // Final state
    clear_screen();
    printf("Knight BFS Traversal Completed:\n\n");
    for (int r = 0; r < 8; r++) 
    {
        for (int c = 0; c < 8; c++) 
        {
            printf(" %c ", board[r][c] == 'K' ? '*' : board[r][c]);
        }
        printf("\n");
    }
    printf("\n");
...
int main()
{
    ...
    for(;;){
        ...
        if(cmd.command == COMMAND_EXIT){
            ...
            if (checkItCanReachAllPoints()) {
                demoKnightMoves();
            }
            else {
                printf("The knight cannot reach all points on the board.\n");
            }
        }
